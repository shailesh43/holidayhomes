...
import 'self_booking_form_screen.dart';

class PropertySelfDetailsScreen extends StatefulWidget {
  ...
  const PropertySelfDetailsScreen({
    super.key,
  ...
  @override
  State<PropertySelfDetailsScreen> createState() => _PropertySelfDetailsScreenState();
}

class _PropertySelfDetailsScreenState extends State<PropertySelfDetailsScreen> {
    ...
    try {
      ...
      if (mounted) {
        Navigator.push(
          context,
          MaterialPageRoute(
            builder: (context) => SelfBookingFormScreen(
      ...
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Error: $e', style: const TextStyle(fontFamily: 'Inter')), backgroundColor: Colors.red));
    } finally {
